...
namespace SD_125_W22SD_Final_Project_UnitTest_Group6
{
    ...
    public class ProjectsBusinessLogicTest
    {
        ...
        [TestMethod]
        public async Task ShouldThrowExceptionWhenAddingAssignedUsersToProjectAndProjectNotFound()
        {
            // Arrange                      
            var testProject = new Project
            {
            ...
            await Assert.ThrowsExceptionAsync<ArgumentException>(async () =>
            {
                await projectsBusinessLogic.AddAssignedUsers(3, testAssignedUsers.Select(u => u.Id).ToList());
            });
            Assert.IsFalse(testProjectUpdated);
        }
    }
